...
(*let reptile_spec = Comp (PathExp (Var "x",File),"x",DirList always)
let reptile_zipper = mk_zipper reptile_spec

let reptiles_spec =
  IPair(PathExp(Name "sneks", cat_spec),
    IPair(PathExp(Name "lizards", cat_spec),
          PathExp(Name "other", cat_spec)
  ))
let reptiles_zipper = mk_zipper reptiles_spec

let others_spec = Comp (PathExp (Var "x",File),"x",DirList always)
let others_zipper = mk_zipper others_spec
*)

let shelter_spec =
  IPair(PathExp(Name "cats", cats_spec),
    PathExp(Name "dogs", dogs_spec)
      (*IPair(PathExp(Name "reptiles", others_spec),
            PathExp(Name "other", others_spec)
  ))*)
  )
