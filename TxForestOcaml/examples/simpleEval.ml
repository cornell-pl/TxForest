...
let main (trans :int) (id: int option) (name:string option) (debug: bool) zipper =
  ...
  | Error s -> p "Error: %s" s; zipper, zipper
...
let () =
  ...
  Command.basic
    ...
    [%map_open
      ...
      in
      if debug then Utils.set_debug () else ();
      ignore_after_f (TxForestCore.loop_txn_noExn d_spec "/Users/katie/Documents/TxForest/example_fs_root/simple" ~f:(main trans id name debug))
    ]
    |> Command.run
